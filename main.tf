resource "helm_release" "ingress_nginx" {
  ...
  version          = var.ingres_nginx_chart_version
  atomic           = var.atomic
  ...
  set {
    ...
    value = var.controller_daemonset_use_hostport
  }
  set {
    name  = "controller.service.externalTrafficPolicy"
    value = var.controller_service_external_traffic_policy
  }
  set {
  ...
  dynamic "set" {
    for_each = local.controller_service_node_ports
    content {
      name  = set.value.name
  ...
  dynamic "set" {
    for_each = local.load_balancer_ip
    content {
      name  = set.value.name
      value = set.value.value
    }
